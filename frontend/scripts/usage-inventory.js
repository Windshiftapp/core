...
function importsOf(file) {
  ...
  // svelte 5: { ..., mount } etc via dynamic loads in .js too (covered above).
  const resolved = [];
...
while (queue.length) {
  ...
  reachable.add(f);
  // traverse forward: everything this file imports
  for (const dep of importsOf(f)) {
    if (!reachable.has(dep)) queue.push(dep);
  }
}

// Files with hardcoded color utilities
const { execSync } = await import('node:child_process');
const out = execSync(
  `rg -l '(?<![-\\w])(?:bg|text)-(?!opacity)[a-z]+-[0-9]{2,3}\\b' ${SRC} --pcre2 -g '!*.test.*' -g '*.svelte' -g '*.ts' -g '*.js' -g '*.css'`,
  { encoding: 'utf8' }
);
...
const countFor = (f) =>
  parseInt(
    execSync(
      `rg -o '(?<![-\\w])(?:bg|text)-(?!opacity)[a-z]+-[0-9]{2,3}\\b' '${f}' --pcre2 | wc -l`
    )
      .toString()
      .trim()
  ) || 0;
...
const NOTES = {
  'lib/utils/cn.js': 'occurrence is a JSDoc example, not rendered UI',
};
...
if (process.argv[2] === '--markdown') {
  const areaOf = (f) => {
    ...
    if (f.startsWith('design-system/viewer/pages/icon-navigation/'))
      return 'Design system viewer (mock pages)';
  ...
  console.log(
    `**${rows.length} files, ${total} occurrences. All files are reachable from an app entry point — no dead code found.**\n`
  );
  console.log(
    `Counts match \`rg '(?<![-\\w])(?:bg|text)-(?!opacity)[a-z]+-[0-9]{2,3}\\b'\` over frontend/src (tests excluded).\n`
  );
  const areas = new Map();
  for (const r of rows) {
...
console.log(
  `Dead (unreachable from any entry): ${dead.length} files, ${dead.reduce((s, r) => s + r.count, 0)} occurrences`
);
