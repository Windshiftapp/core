...
describe('sanitizeHtml', () => {
	...
	it('preserves safe formatting tags', () => {
		...
		expect(sanitizeHtml(input)).toBe('<strong>bold</strong> <em>italic</em> <a href="https://example.com">link</a>');
	});
	...
	it('preserves img tags with safe attributes', () => {
		expect(sanitizeHtml('<img src="photo.jpg" alt="photo">')).toBe('<img src="photo.jpg" alt="photo">');
	});
	...
	it('preserves SVG elements', () => {
		const input = '<svg viewBox="0 0 24 24" width="16" height="16"><circle cx="12" cy="12" r="10" fill="green"></circle></svg>';
		expect(sanitizeHtml(input)).toBe(input);
	});
});
