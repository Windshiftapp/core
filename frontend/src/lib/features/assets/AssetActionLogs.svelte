<script>
  ...
  let { assetSetId, action, onBack } = $props();
  ...
  async function loadLogs() {
    loading = true;
    try {
      logs = await assetActions.getLogs(assetSetId, action.id);
    } catch (err) {
  ...
  const columns = [
    ...
    { key: 'asset_title', label: 'Asset', width: '180px' },
  ...
  <DataTable {columns} data={logs} {loading} pageSize={25} emptyMessage="No execution logs yet">
    {#snippet cell(column, row)}
      ...
      {:else if column.key === 'trigger_event'}
        {row.trigger_event}
      {:else if column.key === 'asset_title'}
        {row.asset_title || '-'}
