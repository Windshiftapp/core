<script>
  ...
  import { getConditionOutputPositions } from '../../actions/nodes/flowDirection.js';

  let { data = {}, selected = false } = $props();
  let condPositions = $derived(getConditionOutputPositions(logbookActionFlowStore.direction));
  let isVertical = $derived(logbookActionFlowStore.direction === 'vertical');
...
<div class="condition-node action-flow-node" class:selected>
  ...
  <div class="node-body">
    ...
    {:else}
      <div class="placeholder">Set condition</div>
    {/if}
  </div>
...
<style>
  ...
  .field { color: var(--ds-text); font-weight: 500; }
  .operator { color: var(--ds-accent-yellow); font-weight: 600; font-size: 14px; }
  .value {
  ...
  .branch-true { color: var(--ds-success); }
  .branch-false { color: var(--ds-danger); }
  .placeholder {
    font-size: 12px;
