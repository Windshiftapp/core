...
<div class="actions-manager">
  <div class="header">
    <div class="header-text">
      <h3>Actions</h3>
      <p class="subtitle">Automate workflows triggered by document events</p>
    </div>
    {#if oncreate}
  ...
  {:else if actions.length === 0}
    <EmptyState
      ...
      description="Create an action to automate workflows when documents are ingested."
    />
  {:else}
        ...
        <div class="action-card">
          <div class="action-info">
            <div class="action-header">
              <span class="status-badge" class:enabled={action.is_enabled} class:disabled={!action.is_enabled}>
          ...
          <div class="action-controls">
            ...
            {/if}
            {#if onviewlogs}
              <Button variant="ghost" size="small" title="View Logs" onclick={() => onviewlogs(action)}>
                <Eye size={14} />
              </Button>
            {/if}
            {#if ontoggle}
              <Button variant="ghost" size="small" title={action.is_enabled ? 'Disable' : 'Enable'} onclick={() => ontoggle(action)}>
