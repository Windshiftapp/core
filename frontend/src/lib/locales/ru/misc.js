...
export default {
  ...
  migrationAssistant: {
    configSetMigration: 'Миграция набора конфигураций', workflowMigration: 'Помощник миграции рабочего процесса', migratingFrom: 'Перенос из', to: 'в', configurationSet: 'Набор конфигураций', analyzingMigration: 'Анализ требований миграции…', analysisFailed: 'Ошибка анализа', noMigrationRequired: 'Миграция не требуется', allItemsCompatible: 'Все элементы ({count}) совместимы с новой конфигурацией.', migrationRequired: 'Требуется миграция', itemsNeedMigration: 'Требуется перенести элементы ({count}). Проверьте сопоставления ниже.', itemTypes: 'Типы элементов', fields: 'Поля', status: 'Статус', priority: 'Приоритет', itemTypeMigrations: 'Миграция типов элементов', customFieldChanges: 'Изменения настраиваемых полей', statusMigrations: 'Миграция статусов', priorityMigrations: 'Миграция приоритетов', noItemsToMigrate: 'Нет элементов для миграции.', noStatusChanges: 'Изменения статусов не обнаружены.', noFieldChanges: 'Изменения полей не обнаружены.', selectTargetType: 'Выберите целевой тип…', selectTargetStatus: 'Выберите целевой статус…', selectTargetPriority: 'Выберите целевой приоритет…', compatible: 'Совместимо', requiresMigration: 'Требует миграции', item: 'элемент', items: 'элементы', kept: 'Сохранено', values: 'значения', hiddenDataPreserved: 'Скрыто (данные сохранены)', newRequiredField: 'Новое обязательное поле', enterDefaultValue: 'Введите значение по умолчанию…', executeMigration: 'Выполнить миграцию', migrating: 'Миграция…', migrationCompleted: 'Миграция завершена', allItemsMigrated: 'Все элементы успешно перенесены.', pleaseSelectTargetStatuses: 'Выберите целевые статусы для всех элементов, требующих миграции.', pleaseSelectTargetItemTypes: 'Выберите целевые типы для всех элементов, требующих миграции.', pleaseProvideDefaultValues: 'Укажите значения по умолчанию для всех новых обязательных полей.', pleaseSelectTargetPriorities: 'Выберите целевые приоритеты для всех элементов, требующих миграции.',
  },
  ...
  scm: {
    ...
    createPullRequest: 'Создать запрос на слияние', createPRFrom: 'Создать PR из ветки {branch}', prTitle: 'Название PR', description: 'Описание', baseBranchPR: 'Базовая ветка', baseBranchPRHelp: 'Оставьте пустым, чтобы использовать основную ветку репозитория.', createPR: 'Создать PR', prCreatedSuccess: 'Запрос на слияние №{prNumber} создан', failedToCreatePR: 'Не удалось создать запрос на слияние', noBranchLink: 'Ссылка на ветку не указана',
    linkDevResource: 'Связать ресурс разработки', linkDevResourceDesc: 'Свяжите PR, ветку или коммит с этим элементом', type: 'Тип', pr: 'PR', branch: 'Ветка', commit: 'Коммит', prNumber: 'Номер PR', commitSha: 'SHA коммита', titleOptional: 'Название (необязательно)', linkResource: 'Связать ресурс', linking: 'Создание связи…', failedToCreateLink: 'Не удалось создать связь',
    development: 'Разработка', linkExisting: 'Связать существующий ресурс', failedToStartConnection: 'Не удалось начать подключение', failedToLoadLinks: 'Не удалось загрузить связи', confirmRemoveLink: 'Удалить эту связь?', noRepositoriesLinked: 'С этим рабочим пространством не связаны репозитории', connectYourAccount: 'Подключите учётную запись {provider}', connectToCreate: 'Подключитесь, чтобы создавать ветки и запросы на слияние из рабочих элементов.', connect: 'Подключить {provider}', noLinksYet: 'Связей с разработкой пока нет', pullRequests: 'Запросы на слияние', branches: 'Ветки', commits: 'Коммиты', smartCommitsTitle: 'Обрабатывать умные коммиты', smartCommitsDescription: 'При слиянии применять директивы «#comment …» и «#<transition-slug>» из описания запроса на слияние и сообщений его коммитов.', smartCommitsWarningTitle: 'Будут приниматься неподтверждённые адреса авторов коммитов', smartCommitsWarningBody: 'Действия умных коммитов выполняются от имени пользователя, чей адрес совпадает с адресом автора в Git. Git не подтверждает это поле: участник может указать любое значение. Включайте только для репозиториев, где доверяете каждому, кто выполняет слияние.', smartCommitsUpdateFailed: 'Не удалось обновить настройку умных коммитов',
  },

  organization: {
    editOrganization: 'Изменить организацию', newOrganization: 'Новая организация', organizationName: 'Название организации', email: 'Электронная почта', organizationAvatar: 'Аватар организации', customAvatar: 'Собственный аватар', uploadedImage: 'Загруженное изображение', defaultAvatar: 'Стандартный аватар', usingInitials: 'Используются инициалы', changeAvatar: 'Изменить аватар', uploadAvatar: 'Загрузить аватар', attachmentsRequired: 'Для загрузки аватара организации необходимо включить вложения', uploadRecommendation: 'Рекомендуется квадратное изображение не меньше 256 × 256 пикселей', activeOrganization: 'Активная организация', customFields: 'Настраиваемые поля', updateOrganization: 'Обновить организацию', createOrganization: 'Создать организацию', pleaseSelectImage: 'Выберите файл изображения', failedToUploadAvatar: 'Не удалось загрузить аватар',
  },
  integrations: {
    title: 'Интеграции', linkPage: 'Связать страницу', searchPages: 'Поиск страниц…', noLinksYet: 'Связанных страниц пока нет', connectAccount: 'Подключите учётную запись', connectToLink: 'Подключитесь, чтобы связывать страницы', connect: 'Подключить', disconnect: 'Отключить', connected: 'Подключено', confirmRemoveLink: 'Удалить эту связь?', confirmDisconnect: 'Отключить эту интеграцию?', failedToLoadLinks: 'Не удалось загрузить связи', failedToSearch: 'Не удалось найти страницы', refreshed: 'Связь обновлена', linked: 'Страница связана', removed: 'Связь удалена', disconnected: 'Интеграция отключена', providerManager: 'Поставщики интеграций', providerManagerDesc: 'Настройка внешних поставщиков интеграций', addProvider: 'Добавить поставщика', editProvider: 'Изменить поставщика', providerType: 'Тип поставщика', oauthClientId: 'ID клиента OAuth', oauthClientSecret: 'Секрет клиента OAuth', callbackUrl: 'URL обратного вызова', callbackUrlHint: 'Скопируйте этот URL в настройки приложения интеграции', providerConfig: 'Конфигурация поставщика (JSON)', notion: 'Notion', noProviders: 'Поставщики интеграций не настроены', selectProvider: 'Выберите поставщика', page: 'Страница', database: 'База данных',
  },
};
