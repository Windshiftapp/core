...
export const issues = {
  ...
  create: (data) => fetchAPI('/issues', {
    method: 'POST',
    body: JSON.stringify(data),
  }),
  update: (id, data) => fetchAPI(`/issues/${id}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  }),
  delete: (id) => fetchAPI(`/issues/${id}`, {
    method: 'DELETE',
  }),
  getComments: (id) => fetchAPI(`/issues/${id}/comments`),
  createComment: (id, data) => fetchAPI(`/issues/${id}/comments`, {
    method: 'POST',
    body: JSON.stringify(data),
  }),
};
...
export const createDiagram = (itemId, name, diagramData) => fetchAPI(`/items/${itemId}/diagrams`, {
  method: 'POST',
  body: JSON.stringify({ name, diagram_data: diagramData }),
});
export const updateDiagram = (diagramId, name, diagramData) => fetchAPI(`/diagrams/${diagramId}`, {
  method: 'PUT',
  body: JSON.stringify({ name, diagram_data: diagramData }),
});
export const deleteDiagram = (diagramId) => fetchAPI(`/diagrams/${diagramId}`, {
  method: 'DELETE',
});

// Comment API functions
export const getComments = (itemId) => fetchAPI(`/items/${itemId}/comments`);
export const createComment = (itemId, data) => fetchAPI(`/items/${itemId}/comments`, {
  method: 'POST',
  body: JSON.stringify(data),
});
export const updateComment = (commentId, data) => fetchAPI(`/comments/${commentId}`, {
  method: 'PUT',
  body: JSON.stringify(data),
});
export const deleteComment = (commentId) => fetchAPI(`/comments/${commentId}`, {
  method: 'DELETE',
});
...
export const attachments = {
  ...
  // Delete attachment
  delete: (attachmentId) => fetchAPI(`/attachments/${attachmentId}`, {
    method: 'DELETE',
  }),
};
...
export const attachmentSettings = {
  ...
  update: (id, data) => fetchAPI(`/attachment-settings/${id}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  }),
...
export const calendarFeed = {
  ...
  createToken: () => fetchAPI('/calendar/feed/token', {
    method: 'POST',
  }),

  // Revoke feed token
  revokeToken: () => fetchAPI('/calendar/feed/token', {
    method: 'DELETE',
  }),
};
...
export const personalLabels = {
  ...
  create: (data) => fetchAPI('/personal-labels', {
    method: 'POST',
    body: JSON.stringify(data),
  }),
  update: (id, data) => fetchAPI(`/personal-labels/${id}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  }),
  delete: (id) => fetchAPI(`/personal-labels/${id}`, {
    method: 'DELETE',
  }),
};
...
export const jiraImport = {
  ...
  testConnection: (data) => fetchAPI('/jira-import/connect', {
    method: 'POST',
    body: JSON.stringify(data),
  }),
  // Get available Jira projects
  getProjects: (connectionId, openIssuesOnly = false) => fetchAPI(`/jira-import/projects?connection_id=${connectionId}&open_issues_only=${openIssuesOnly}`),
  // Analyze selected projects
  analyzeProjects: (connectionId, projectKeys, openIssuesOnly = false) => fetchAPI('/jira-import/analyze', {
    method: 'POST',
    body: JSON.stringify({
  ...
  getAssetTypes: (connectionId, schemaId) => fetchAPI(`/jira-import/assets/${schemaId}/types?connection_id=${connectionId}`),
  // Get import job status
  getJobStatus: (jobId) => fetchAPI(`/jira-import/jobs/${jobId}`),
  // Delete connection
  deleteConnection: (connectionId) => fetchAPI(`/jira-import/connections/${connectionId}`, {
    method: 'DELETE',
  }),
  // Start import
  startImport: (data) => fetchAPI('/jira-import/start', {
    method: 'POST',
    body: JSON.stringify(data),
  }),
};
