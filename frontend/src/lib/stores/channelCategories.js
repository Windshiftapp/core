...
function createChannelCategoriesStore() {
  ...
  return {
    ...
    async init() {
      try {
        const categories = await api.channelCategories.getAll();
        set(categories || []);
        return categories || [];
      } catch (error) {
    ...
    async add(categoryData) {
      try {
        ...
        const newCategory = await api.channelCategories.create({
    ...
    async update(categoryId, updates) {
      try {
        const updatedCategory = await api.channelCategories.update(categoryId, updates);
    ...
    async delete(categoryId) {
      try {
        await api.channelCategories.delete(categoryId);
...
export const channelCategoriesStore = createChannelCategoriesStore();
