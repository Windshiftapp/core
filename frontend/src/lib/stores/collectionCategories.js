...
function createCollectionCategoriesStore() {
  ...
  return {
    ...
    async init() {
      try {
        const categories = await api.collectionCategories.getAll();
        set(categories || []);
        return categories || [];
      } catch (error) {
    ...
    async add(categoryData) {
      try {
        ...
        const newCategory = await api.collectionCategories.create({
        ...
        console.error('Failed to add collection category:', error);
        throw error;
      }
    },
    ...
    async update(categoryId, updates) {
      try {
        const updatedCategory = await api.collectionCategories.update(categoryId, updates);
        ...
        console.error('Failed to update collection category:', error);
        throw error;
      }
    },
    ...
    async delete(categoryId) {
      try {
        await api.collectionCategories.delete(categoryId);
        ...
        console.error('Failed to delete collection category:', error);
        throw error;
      }
    },
...
export const collectionCategoriesStore = createCollectionCategoriesStore();
