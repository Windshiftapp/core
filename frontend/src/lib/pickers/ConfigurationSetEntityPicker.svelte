<script>
  ...
  function addEntity(entityId) {
    ...
    if (entityAssignments[entityId]) {
      ...
      if (!confirm(`This ${getEntityLabel()} is already assigned to "${assignment.configSetName}". Assigning it here will remove it from that configuration set. Continue?`)) {
        return;
      }
    }
  ...
  function getEntityLabel() {
    switch (entityType) {
      ...
      case 'item-types': return 'item type';
  ...
  function getEntityLabelPlural() {
    switch (entityType) {
      ...
      case 'item-types': return 'item types';
...
<div class="space-y-4">
  ...
  <div class="relative">
    ...
    <input
      type="text"
      placeholder="Search {getEntityLabelPlural()}..."
  ...
  <div>
    <h4 class="text-sm font-medium mb-2" style="color: var(--ds-text);">
      Assigned ({assignedEntities.length})
    </h4>
    {#if assignedEntities.length === 0}
      <div class="p-4 text-center border rounded-lg" style="border-color: var(--ds-border); color: var(--ds-text-subtle);">
        No {getEntityLabelPlural()} assigned
      </div>
    {:else}
  ...
  <div>
    ...
    {#if availableEntities.length === 0}
      <div class="p-4 text-center border rounded-lg" style="border-color: var(--ds-border); color: var(--ds-text-subtle);">
        {#if searchQuery}
          No {getEntityLabelPlural()} match your search
        {:else}
          All {getEntityLabelPlural()} are assigned
        {/if}
      </div>
    {:else}
            ...
            <div class="flex items-center gap-3 min-w-0 flex-1">
              ...
              {#if otherAssignment}
                <div class="flex items-center gap-1 text-xs px-2 py-1 rounded flex-shrink-0" style="background-color: var(--ds-surface-warning); color: var(--ds-text-warning);">
                  ...
                  <span class="truncate max-w-32">In {otherAssignment.configSetName}</span>
                </div>
              {/if}
            </div>
            <button
