<script>
  ...
  const resolvedPlaceholder = $derived(placeholder || 'Select portal customer');
  const resolvedUnassignedLabel = $derived(unassignedLabel || 'None');

  // Load portal customers
  ...
  let selectedCustomer = $derived(
    (customers.data || []).find(c => c.id === value) || null
  );

  // Filter and limit customers
  let filteredCustomers = $derived.by(() => {
    let result = customers.data || [];
    if (searchTerm.trim()) {
      const term = searchTerm.toLowerCase();
      result = result.filter(c =>
        c.name?.toLowerCase().includes(term) ||
        c.email?.toLowerCase().includes(term) ||
        c.customer_organisation_name?.toLowerCase().includes(term)
      );
    } else {
      // Show only 4 customers by default (when no search)
  ...
  function handleKeyDown(e) {
    const totalItems = (showUnassigned ? 1 : 0) + filteredCustomers.length;
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (showUnassigned && highlightedIndex === 0) {
        ...
        if (adjustedIndex >= 0 && adjustedIndex < filteredCustomers.length) {
          handleSelect(filteredCustomers[adjustedIndex]);
        }
      }
    ...
    <div class="flex items-center gap-2 flex-1 min-w-0">
      {#if selectedCustomer}
        <div class="w-6 h-6 rounded-full bg-orange-100 flex items-center justify-center">
          <User size={14} class="text-orange-600" />
        </div>
  ...
  >
    ...
    <div class="p-2 border-b" style="border-color: var(--ds-border);">
      <input
        ...
        type="text"
        placeholder="Search portal customers..."
    ...
    <!-- Customers List -->
    <div class="max-h-80 overflow-y-auto" role="listbox" id={listboxId} aria-label="Portal customers">
      ...
      {:else}
          ...
          >
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center flex-shrink-0">
                <User size={16} class="text-orange-600" />
              </div>
              <div class="flex flex-col min-w-0 flex-1">
                <span class="font-medium truncate" style="color: var(--ds-text);">
                  {customer.name}
                </span>
                <Text size="xs" variant="subtle" truncate>{customer.email}</Text>
                {#if customer.customer_organisation_name}
                  <Text size="xs" variant="subtle" truncate>{customer.customer_organisation_name}</Text>
                {/if}
              </div>
            </div>
          </button>
        ...
        <!-- No Results -->
        {#if filteredCustomers.length === 0 && !showUnassigned}
          <div class="p-4 text-center" style="color: var(--ds-text-subtle);">
