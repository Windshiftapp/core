...
func (h *AgentHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if !isAdmin && !h.allowUserManagedAgents() {
		_ = logger.LogAudit(h.db, logger.AuditEvent{
			...
			Success:      false,
			ErrorMessage: "user-managed agents are disabled",
		})
	...
	if !isAdmin {
		...
		count, err := h.countOwnedAgents(currentUser.ID)
		if err != nil {
		...
		if count >= maxAgents {
			_ = logger.LogAudit(h.db, logger.AuditEvent{
				...
				Success:      false,
				ErrorMessage: "agent limit reached",
			})
			respondForbidden(w, r)
			return
		}
	...
	`, email, req.Username, req.FirstName, req.LastName, currentUser.ID, now, now).Scan(&newID)
	if err != nil {
	...
	agent := models.User{
		...
		IsActive:         true,
		IsAgent:          true,
		AgentOwnerUserID: &currentUser.ID,
