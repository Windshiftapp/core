...
func RegisterAIRoutes(deps *Deps) {
	...
	// AI feature endpoints (user)
	api.HandleH("GET /ai/status", auth(http.HandlerFunc(deps.AI.AI.Status)))
	api.HandleH("GET /ai/plan-my-day", auth(http.HandlerFunc(deps.AI.AI.PlanMyDay)))
	api.HandleH("POST /ai/items/{id}/catch-me-up", auth(http.HandlerFunc(deps.AI.AI.CatchMeUp)))
	api.HandleH("POST /ai/items/{id}/find-similar", auth(http.HandlerFunc(deps.AI.AI.FindSimilarItems)))
	api.HandleH("POST /ai/items/{id}/decompose", auth(http.HandlerFunc(deps.AI.AI.DecomposeItem)))
